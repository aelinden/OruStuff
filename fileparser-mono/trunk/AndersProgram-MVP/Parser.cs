...
namespace OruSharpReader
{
	...
	/// be refactored at some point since it is way to big and complicated.
	/// </summary>
	public class Parser
	{
		...
		public void setIOFiles(string iFile, string oFile)
		{
			inputFile = iFile;
			outputFile = oFile;
			fh.SetIO(iFile, oFile);
		}
						...
						if(currLineSplit[classCol].Equals(c.ClassId))
						{
							for(int i = qColBegin; i < numCols; ++i)
							{
								...
								if(currLineSplit[i].Equals(c.StudentId) 
									&& !currLineSplit[idCol].Equals(c.StudentId) 
									&& currLineSplit[idCol].Length != 0)
								{
